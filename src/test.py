...
async def test_frame0(dut):
    ...
    print("--- TEST DEBUG: Testing first line...")
    # First, compensate for 1-clock delay due to registered RGB outputs:
    await ClockCycles(dut.clk, 1)
    ...
    for h in range(640):
        ...
        await ClockCycles(dut.clk, 1)
    # HFRONT porch:
    for h in range(HF-1):  #NOTE: HF-1, because following it is HSYNC which will APPEAR to be 1 clock early, because it is not registered.
        # Expect black:
    ...
    # # The first 2 lines of EVERY frame should be yellow, i.e. R&G both asserted for 640 clocks,
    # # then nothing for the duration of the horizontal front porch, then hsync asserted, then
    # # horizontal back porch, and then it should repeat again.
    # # First, though, compensate for 1-clock delay due to registered RGB outputs:
    # await ClockCycles(dut.clk, 1)
    # for h in range(640):
    #     # Expect yellow:
    #     assert dut.red.value    == 1
    ...
    #     # Advance to the next clock:
    #     await ClockCycles(dut.clk, 1)
    # # HBLANK area at the end of the line:
    # for h in range(HF+HS+HB):
    #     assert dut.red.value    == 0, f"Red channel shouldn't be asserted at h:{640+h}"
    #     assert dut.green.value  == 0, f"Green channel shouldn't be asserted at h:{640+h}"
    #     assert dut.blue.value   == 0, f"Blue channel shouldn't be asserted at h:{640+h}"
    #     if h<=HF or h>=HF+HS:
    #         assert dut.hsync.value == 1
    #     else:
    #         assert dut.hsynv.value == 0
